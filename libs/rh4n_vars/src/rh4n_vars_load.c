...
int rh4nvarsLoadNode(int inputfile, RH4nVarList *varlist) {
    ...
    while(1) {
        //End of file
        if(read(inputfile, &namelength, sizeof(namelength)) == 0) {
            break;
        ...
        if((rh4nvarsLoadValue(inputfile, &newNode->var)) != RH4N_RET_OK) return(ret);
...
void rh4nvarsPutInNewNode(char next, char nextlvl, RH4nVarEntry_t *newNode, RH4nVarEntry_t *lastNode, RH4nVarList *varlist) {
    ...
    if(nextlvl == 1) {
        ...
        for(hptr = lastNode; hptr != NULL && hptr->var.type != RH4NVARTYPEGROUP; hptr = hptr->prev);
        if(hptr == NULL) return;
