...
int json_aton_handling(int natparm_pos, struct parameter_description natvar_pd, 
    ...
    if((rc = nnifuncs->pf_nni_from_string(nnifuncs, buff, NNI_TYPE_NUM, natvar_pd.length+natvar_pd.precision, 
            natvar_pd.precision, sizeof(natbuff), (void*)natbuff)) != NNI_RC_OK) {
        printf("NNI from string: [%d]\n", rc);
        free(buff);
        return(RH4N_RET_NNI_ERR);
    }
    
    if((rc = nnifuncs->pf_nni_put_parm(nnifuncs, natparm_pos, parmhandle, natvar_pd.length+natvar_pd.precision, 
            natbuff)) != NNI_RC_OK) {
        if(rc < 0) {
            free(buff);
            return(RH4N_RET_NNI_ERR);
...
int json_1daton_handling(int natparm_pos, struct parameter_description natvar_pd, 
    pnni_611_functions nnifuncs, void* parmhandle, vars_t *target) {
    ...
    for(; x < target->x_length; x++) {
        ...
        if((rc = nnifuncs->pf_nni_put_parm_array(nnifuncs, natparm_pos, parmhandle, natvar_pd.length+natvar_pd.precision, 
                natbuff, occ)) != NNI_RC_OK) {
            if(rc < 0) {
                free(buff);
                return(RH4N_RET_NNI_ERR);
            }
...
int json_2daton_handling(int natparm_pos, struct parameter_description natvar_pd, 
    pnni_611_functions nnifuncs, void* parmhandle, vars_t *target) {
...
int json_3daton_handling(int natparm_pos, struct parameter_description natvar_pd, 
    pnni_611_functions nnifuncs, void* parmhandle, vars_t *target) {
